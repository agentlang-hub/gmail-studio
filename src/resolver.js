...
const isSim = getLocalEnv('AL_GMAIL_SIM');
...
if (isSim) {
    ...
    simulatedEmails = JSON.parse(data);
}

// Mapper functions for Gmail API responses to Agentlang entities
function toEmail(messageDetail, headers) {
    if (isSim && simulatedEmails.length > 0) {
	const i = Math.floor(Math.random() * simulatedEmails.length)
	return simulatedEmails[i]
    }
...
const makeRequest = async (endpoint, options = {}) => {
	...
	if (error.name === "AbortError") {
	    console.error(
		`GMAIL RESOLVER: Request timeout - ${url} - ${JSON.stringify(options)}`,
	    );
	} else if (
...
export const updateEmail = async (env, attributes, newAttrs) => {
	...
	if (removeLabels) {
	    data.removeLabelIds = removeLabels
		.split(",")
...
export const createLabel = async (env, attributes) => {
	...
	attributes.attributes.get("message_list_visibility") || "show",
	labelListVisibility:
	attributes.attributes.get("label_list_visibility") || "labelShow",
...
export const fetchAttachment = async (env, attributes) => {
	...
	const messages = await makeGetRequest(
...
async function getAndProcessRecords(resolver, entityType) {
	...
	console.error(
	    `GMAIL RESOLVER: Failed to process ${entityType} records: ${error}`,
	);
    }
}
